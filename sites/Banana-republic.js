    ...
    return await page.evaluate(() => {
        ...
        const formated = Array.from(products).map(product => {
            ...
            if (salesPage) {
                ...
                price = product.querySelector(".product-price__regular")
            }

            if (!price) {
                console.log("missgin price", price, product)
            } else if (!title) {
                console.log("missgin title")
            }
...
module.exports = async (url, proxy) => {
    ...
    if (page.evaluate(() => { document.body.innerHTML.includes("Access Denied") })) {
        await browser.close();
        return []
    }
